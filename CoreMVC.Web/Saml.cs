...
public abstract class BaseResponse
{
    ...
    public string Xml { get { return _xmlDoc?.OuterXml ?? string.Empty; } }

    public BaseResponse(string certificateStr, string? responseString = null)
    {
        var pem = EnsureCertFormat(certificateStr);
        _certificate = X509Certificate2.CreateFromPem(pem);
    ...
    {
        var pem = Encoding.ASCII.GetString(certificateBytes);
        pem = EnsureCertFormat(pem);
        _certificate = X509Certificate2.CreateFromPem(pem);
        if (responseString != null)
            LoadXmlFromBase64(responseString);
    }
    ...
    protected bool ValidateSignatureReference(SignedXml signedXml)
    {
        if (_xmlDoc == null || _xmlNameSpaceManager == null)
            return false;
        // capture locals for flow analysis
        var xmlDoc = _xmlDoc;
        var ns = _xmlNameSpaceManager;

        var references = signedXml.SignedInfo?.References;
        if (references == null || references.Count != 1) //no ref or not exactly one
            return false;
        ...
        var id = reference.Uri is null || reference.Uri.Length == 0 ? string.Empty : reference.Uri.Substring(1);
        ...
        {
            var assertionNode = xmlDoc.SelectSingleNode("/samlp:Response/saml:Assertion", ns) as XmlElement;
    ...
    {
        if (_xmlDoc == null)
            throw new InvalidOperationException("XML document not loaded");
    ...
    public bool IsValid()
    {
        ...
        var first = nodeList[0] as XmlElement;
        if (first == null) return false;

        SignedXml signedXml = new SignedXml(_xmlDoc);
        signedXml.LoadXml(first);
    ...
    private bool IsExpired()
    {
        ...
        var notOnOrAfter = node?.Attributes?["NotOnOrAfter"]?.Value;
        if (!string.IsNullOrEmpty(notOnOrAfter))
        {
            DateTime.TryParse(notOnOrAfter, out expirationDate);
        }
...
public class Response : BaseResponse
{
    ...
    public string? GetNameID()
    {
        ...
        XmlNode? node = _xmlDoc.SelectSingleNode("/samlp:Response/saml:Assertion[1]/saml:AttributeStatement/saml:Attribute[@Name='NameID']/saml:AttributeValue", _xmlNameSpaceManager);
        return node?.InnerText;
    }
...
public class SignoutResponse : BaseResponse
{
    ...
    public string? GetLogoutStatus()
    {
        ...
        return val?.Replace("urn:oasis:names:tc:SAML:2.0:status:", string.Empty);
    }
}
